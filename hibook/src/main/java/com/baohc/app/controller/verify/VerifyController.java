	...
	private void doVerifyOTP(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (otp_user.equals(code_otp)) {
				...
				String mess = "";
				boolean register = authN.regiter(userDeliveried.getUsername(), userDeliveried.getEmail(),
						userDeliveried.getPassword(), confirmPassword, userDeliveried.getFullname(), birthDate, request,
						mess);
				if (register) {
					...
					resp.put("status", "success");
					resp.put("message", "Đã email xác minh thành công!" + mess);

				} else {
					// Registration thất bại
					resp.put("status", "error");
					resp.put("message", "Tạo tài khoản thất bại!" + mess);
...
//			}
	...
	private String getHTMLTemplate(String otpCode) {
		...
		html.append("<!DOCTYPE html>").append("<html lang=\"vi\">").append("<head>").append("<meta charset=\"UTF-8\">")
				.append("<title>Xác thực email - HiBOOK</title>").append("</head>")
				...
				.append("Xác thực tài khoản của bạn").append("</td></tr>")
				...
				.append("Vui lòng sử dụng mã OTP bên dưới để hoàn tất xác thực:").append("</td></tr>")
				...
				.append(otpCode).append("</div></td></tr>")
				.append("<tr><td align=\"center\" style=\"color:#ef4444;font-size:14px;padding:15px 0;\">")
				.append("Mã OTP chỉ có hiệu lực trong vòng <b>1 phút</b>.").append("</td></tr>")
				...
				.append("© 2025 HiBOOK.com. All rights reserved.").append("</td></tr>")
	...
	private void handelInvalidRequest(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
