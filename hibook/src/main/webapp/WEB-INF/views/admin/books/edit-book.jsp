		...
		<div class="row justify-content-evenly">
			<div class="col-md-5 p-3">
				...
				<div class="wrapper bg_dark-blue-950-o50 p-3 mb-3 rounded-3">
					...
					<c:set var="book" value="${sessionScope.book }"></c:set>
					<input type="hidden" value="${book.id }" id="bookId" readonly="readonly">
					<div class="mb-3">
						<label class="form-label" for="bookname">Tên sách</label>
						<input type="text" class="input-custom_border" id="bookname" name="bookname" placeholder="Nhập vào tên sách" value="${book.name }">
					</div>
						...
						<div class="filter-category ">
							<select class="input-custom_border mx-0 border-0" id="author" name="author">
								<c:forEach var="author" items="${sessionScope.authorList  }">
									<option value="${author.id }" <c:if test="${author.id == book.author.id }">selected</c:if> >${author.fullname}</option>
								</c:forEach>
							</select>
						</div>
					</div>
					...
					<div class="mb-3">
						...
						<textarea class="input-custom_border" id="description" name="description" style="height: 100px" placeholder="Nhập vào mô tả">${book.description }</textarea>
					</div>
				...
				<div class="wrapper bg_dark-blue-950-o50 p-3 mb-3 rounded-3">
					...
					<div class="mb-3">
						...
						<input type="text" class="input-custom_border" id="price" name="price" placeholder="Nhập vào giá" value="${book.price }">
					</div>
					
					<div class="mb-3">
						<label class="form-label" for="amount">Số lượng</label>
						<input type="number" class="input-custom_border" id="amount" name="amount" placeholder="Nhập vào số lượng" value="${book.amount }">
					</div>
				</div>	
				...
				<form id="form-uploadFile" action="${pageContext.request.contextPath }/UploadMutipleFile" method="post" enctype="multipart/form-data">
					<div class="d-flex flex-column mb-3">
						...
						<div class="text-center">
							<img  id="coverPhoto-preview" alt="" src="${pageContext.request.contextPath }/assets/images/books/${coverPhotoPathName}" width="200" class="rounded-2 object-fit my-3">
						</div>
						<input type="hidden" name="oldCoverPhoto" value="${coverPhotoPathName}">
					</div>
					
					<div class="d-flex flex-column mb-3">
						<label class="form-label" for="photo">Ảnh bổ sung</label>	
						<input type="file" class="form-control d-none" id="photo" name="photo" multiple="multiple" value="">
						...
						<div id="preview" class="d-flex justify-content-evenly flex-wrap mt-3">
							...
							<c:forEach var="photo" items="${sessionScope.photoList }">
								<img src="${pageContext.request.contextPath }/assets/images/books/${photo.pathname}" class="rounded-2" width="100">
								 <input type="hidden" name="oldPhotos" value="${photo.pathname}">
							</c:forEach>
						</div>
					</div>
				</form>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#form-AddBook").on("submit", function(e){
				e.preventDefault(); 
				console.log("form edit book active");
				...
				let txtBookId = $("#bookId").val();
				
				//Ajax upload book info
				$.ajax({
					url: "${pageContext.request.contextPath }/admin/book/edit",
					method: "POST",
					dataType: "JSON",
					data: {
					...
					success: function(response) {
						if(response.status === "success") {
							...
							$.ajax({
								url: "${pageContext.request.contextPath }/uploadfile/edit-book",
								method: "POST",
								dataType: "json",
								data: formData,
								processData: false,
								...
								success: function(resUpload) {
									if (resUpload.status === "success") {
										Swal.fire({
				                    		...
				                    		<c:remove var="coverPhotoPathName" scope="session" />
				                    		<c:remove var="photoList" scope="session" />
